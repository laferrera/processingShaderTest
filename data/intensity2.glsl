...
void main()
{
    ...
    // const float buckets = float(resolution.x);
    ...
    const float gain = 0.5; // 0.3
    const float blend = 0.7; // 0.6 higher means more white
    count.rgb = log( mix(count.rgb, count.aaa, blend) ) * gain;
          
    // output
    // this works... 
    // vec4 pixel=texture2D(texture,vec2(uv.x,320.0/resolution.y));
    // gl_FragColor=pixel;
    gl_FragColor=count;
